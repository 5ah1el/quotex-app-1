...
class AdvancedOTCPredictor:
    ...
    async def predict(self, symbol, timeframe, candle_history):
        """Advanced OTC prediction with precise timing"""
        if len(candle_history) < self.min_data_points:
            return {
                ...
                'time_left': candle_history[-1].get('time_left', 0) if candle_history else 0
            }
        
        # Get the last candle's time_left for countdown
        last_candle = candle_history[-1]
        time_left = last_candle.get('time_left', 0)
        
        # Only analyze completed candles for accurate signals
        if not last_candle.get('is_closed', False) and time_left > 5:
            # Still forming, wait until closer to close
            return {
                'symbol': symbol,
                'timeframe': timeframe,
                'direction': 'WAITING',
                'confidence': 0.0,
                'reason': f'Analyzing... Next signal in {time_left}s',
                'time_left': time_left
            }
    ...
    def _pattern_recognition(self, close, high, low):
        ...
        # Bullish Engulfing
        if len(close) >= 2:
            open_prev = close[-3]  # Approximation
            open_curr = close[-2]
        ...
        # Reversal pattern (2-candle)
        if len(close) >= 3:
            change1 = close[-2] - close[-3]
            change2 = close[-1] - close[-2]
            
            if change1 < 0 and change2 > 0 and abs(change2) > abs(change1):
                bullish += 2
                reasons.append("Strong reversal upward")
            elif change1 > 0 and change2 < 0 and abs(change2) > abs(change1):
                bearish += 2
                reasons.append("Strong reversal downward")
